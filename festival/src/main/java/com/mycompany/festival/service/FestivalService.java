...
//        }
    ...
    public List<FestivalO> findAllFestivals() {
        // Παίρνουμε όλες τις οντότητες Festival από τη βάση
        List<Festival> festivalEntities = festivalRepository.findAll();

        // Τις μετατρέπουμε σε FestivalO (DTOs) για να τις στείλουμε στον client
        return festivalEntities.stream()
                .map(this::convertEntityToDto) // Χρειαζόμαστε μια βοηθητική μέθοδο
                .collect(Collectors.toList());
    }

    // ΒΟΗΘΗΤΙΚΗ ΜΕΘΟΔΟΣ ΓΙΑ ΜΕΤΑΤΡΟΠΗ
    private FestivalO convertEntityToDto(Festival festival) {
        FestivalO dto = new FestivalO();
        dto.setId(festival.getId());
        dto.setName(festival.getName());
        dto.setDescription(festival.getDescription());
        dto.setStartDate(festival.getStartDate());
        dto.setEndDate(festival.getEndDate());
        dto.setState(festival.getState());
        // Πρόσθεσε κι άλλα πεδία αν χρειάζεται
        return dto;
    }
